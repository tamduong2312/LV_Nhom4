...
public class CTToaThuoc {
    ...
    private int maThuoc;
    private String tenThuoc;    // Để hiển thị
    private String donViTinh;   // Để hiển thị
    private double donGia;      // Để tính tiền (không liên quan kho)
    ...
    private double parseDose(String text) {
        ...
        try {
            // Xử lý trường hợp "1/2"
            if (text.contains("/")) {
                String[] parts = text.split("/");
                return Double.parseDouble(parts[0]) / Double.parseDouble(parts[1]);
            }
            // Lấy số từ chuỗi (VD: "2 viên" -> 2)
