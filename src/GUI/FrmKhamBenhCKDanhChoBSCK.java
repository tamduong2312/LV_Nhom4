...
public class FrmKhamBenhCKDanhChoBSCK extends JFrame {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(() -> {
            try {
                ...
                FrmKhamBenhCKDanhChoBSCK frame = new FrmKhamBenhCKDanhChoBSCK("");
    ...
    public FrmKhamBenhCKDanhChoBSCK(String tenbn) {
        setTitle("Giao diện Bác sĩ chuyên khoa");
        this.tenbn = tenbn;
        ...
        btnXemChiSo.addActionListener(e -> {
            ...
            FrmXemChiSoTongQuatCuaBSTQ content = new FrmXemChiSoTongQuatCuaBSTQ(Session.maphieukham);
        ...
        tabbedPane.addTab("Khám Tổng Quát", null, new FrmKhamLamSangNhiKhoa(Session.maphieukham), "Thông tin khám sơ bộ");

        int maCK = Session.machuyenkhoa;
        
        switch (maCK) {
            case 3: 
                tabbedPane.addTab("Khám Nhi", null, new PnlKhamNhiKhoa(Session.maphieukham), null);
                break;
            case 4: 
                tabbedPane.addTab("Tai - Mũi - Họng", null, new PnlKhamTaiMuiHong(Session.maphieukham), null);
                break;
            case 5: 
                tabbedPane.addTab("Răng - Hàm - Mặt", null, new PnlKhamRangHamMat(Session.maphieukham), null);
                break;
            case 11: 
                tabbedPane.addTab("Tim Mạch", null, new PnlKhamTimMach(Session.maphieukham), null);
                break;
            default:
                break;
        }
...
class PnlKhamNhi extends JPanel {
    public PnlKhamNhi(int id) {
        add(new PnlKhamNhiKhoa(Session.maphieukham));
        setBackground(Color.WHITE);
    }
}

class PnlTaiMuiHong extends JPanel {
    public PnlTaiMuiHong(int id) {
        add(new JLabel("GIAO DIỆN NỘI SOI TAI MŨI HỌNG (Mã Phiếu: " + id + ")"));
        setBackground(Color.WHITE);
    }
}

class PnlRangHamMat extends JPanel {
    public PnlRangHamMat(int id) {
        add(new JLabel("GIAO DIỆN KHÁM RĂNG HÀM MẶT (Mã Phiếu: " + id + ")"));
        setBackground(Color.WHITE);
    }
}

class PnlTimMach extends JPanel {
    public PnlTimMach(int id) {
        add(new JLabel("GIAO DIỆN KHÁM TIM MẠCH (Mã Phiếu: " + id + ")"));
        setBackground(Color.WHITE);
    }
}