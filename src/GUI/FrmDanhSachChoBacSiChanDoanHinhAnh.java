...
public class FrmDanhSachChoBacSiChanDoanHinhAnh extends JFrame {
    ...
    public FrmDanhSachChoBacSiChanDoanHinhAnh() {
        ...
        table = new JTable();
        String[] cols = {"ID CT", "Mã BN", "Tên Bệnh Nhân", "Năm sinh", "Giới tính", "Dịch vụ cần làm","Loại dịch vụ", "BS Chỉ định", "Thời gian", "Trạng thái"};
        ...
        btnThucHien.addActionListener(e -> {
            ...
            String loaiDV = table.getModel().getValueAt(index, 6).toString();
            ...
            if (loaiDV.equalsIgnoreCase("CLS_CHAN_DOAN_HINH_ANH")) {
                ...
                Session.maphieukham = bus.getMaPhieuKhamByBenhNhan(mabn);
                contentPanel = new PnlNhapKqChuanDoanHinhAnh(idChiTiet, tenBN, tenDV);
                dialog.setSize(950, 650);
                
            } else if (loaiDV.equalsIgnoreCase("CLS_XET_NGHIEM")) {
                PhieuKhamBus bus= new  PhieuKhamBus();
                Session.maphieukham = bus.getMaPhieuKhamByBenhNhan(mabn);
                PnlXetNghiem q = new PnlXetNghiem(idChiTiet);
                q.setVisible(true);
                dispose();
                return;
            } else {
    ...
    private void loadData() {
        List<DanhSachChoCLS> list = chiDinhBus.getDanhSachChoCLS();
