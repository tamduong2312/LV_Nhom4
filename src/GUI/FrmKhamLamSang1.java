...
public class FrmKhamLamSang1 extends JPanel {
        ...
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    ...
                    FrmKhamLamSang1 panel = new FrmKhamLamSang1( mapk,tenbn);
     ...
     * Create the panel.
     */
    public FrmKhamLamSang1(int mapk, String tenbn) {
    	this.mapk = mapk;
    	this.tenbn = tenbn;
        ...
        // --- SỬA Ở ĐÂY: KHỞI TẠO COMBOBOX & ÁP DỤNG AUTOCOMPLETE ---
        txtChanDoan = new JComboBox<>();
        txtChanDoan.setEditable(true); // Cho phép nhập liệu để tìm kiếm
        txtChanDoan.setBackground(Color.WHITE);
        txtChanDoan.setBounds(20, 485, 870, 50); // Giữ nguyên vị trí kích thước
        ...
        btnHoanTatKham.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		PhieuKhamBus bus = new PhieuKhamBus();
        		bus.updateTrangThaiHoantatKhamBenh(Session.maphieukham);
    ...
    private void xuatPhieuKhamPDF() {
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                // Thông tin hành chính (Nên lấy thêm tên BN từ Bus nếu có thể)
                ...
                // Nội dung khám bệnh (Lấy từ các TextField/TextArea trên màn hình)
                ...
                // Chẩn đoán (Lấy từ Editor của ComboBox)
                ...
                pSign.setIndentationRight(50); // Thụt lề phải cho đẹp
                document.add(pSign);
    ...
    // Hàm phụ trợ để thêm từng mục cho gọn code
    ...
    private void initEvents() {
        btnLuu.addActionListener(e -> {
            ...
            try {
                ...
                // --- SỬA LOGIC LƯU: Lấy text từ ComboBox (vì là Editable) ---
        ...
        ActionListener comboListener = e -> {
            ...
            if (selectedObj != null && !selectedObj.toString().startsWith("--")) {
                ...
                String prefix = (source == cboXN) ? "- XN: " : (source == cboCDHA) ? "- CĐHA: " : "- TDCN: ";
    ...
    public void loadDataCu() {
        if ( mapk <= 0) return;
        KhamLamSangBus bus = new KhamLamSangBus();
        KhamLamSang kls = bus.getByMaPhieuKham(mapk);
        if (kls != null) {
            txtLyDo.setText(kls.getLyDoKham());
            txtBenhSu.setText(kls.getBenhSu());
            txtTienSu.setText(kls.getTienSuBanThan());
            txtKhamLamSang.setText(kls.getKhamLamSang());
            
            // --- SỬA LOGIC LOAD: Set item cho ComboBox ---
            txtChanDoan.setSelectedItem(kls.getChanDoanSoBo());
            // ---------------------------------------------
