...
public class FrmDanhSachChoKhamTroLyTQ extends JFrame {
    ...
    public FrmDanhSachChoKhamTroLyTQ() {
        ...
        String[] columns = {"ID", "Mã BN", "Bệnh nhân", "Mã CK", "Chuyên khoa", "STT", "Thời gian", "Trạng thái", "Ghi chú"};
        ...
        JButton btnVangMat = new RoundedButton("Đánh dấu vắng mặt", DANGER_COLOR); 
        btnVangMat.setPreferredSize(new Dimension(180, 40));
        
        JButton btnDangKySTTMoi = new RoundedButton("Đăng ký STT Mới", new Color(255, 215, 0)); 
        ...
        btnKham.addActionListener(e -> {
            ...
            int idDangKy = (int) table.getValueAt(index, 0); 
            ...
            int maPhieuHienTai = bus.getMaPhieuKhamTrongNgay(Session.mabenhnhan);
            
            if (maPhieuHienTai > 0) {

                Session.maphieukham = maPhieuHienTai;
                System.out.println("Dùng lại phiếu cũ: " + maPhieuHienTai);
            } else {
    
                int maPhieuMoi = bus.TroLyTQTaoPhieuKham(Session.mabenhnhan, Session.maNhanVien, Session.chuyenKhoa);
                if (maPhieuMoi > 0) {
                    Session.maphieukham = maPhieuMoi;
                    System.out.println("Tạo phiếu mới: " + maPhieuMoi);
                    
           
                    DangKyKhamBenhBus u = new DangKyKhamBenhBus();
                    u.capNhatTrangThai(idDangKy); 
                } else {
                    JOptionPane.showMessageDialog(null, "Lỗi khi tạo phiếu khám!");
                    return;
                }
            }
            ...
            FrmNhapChiSoSinhTon contentPanel = new FrmNhapChiSoSinhTon(idDangKy, Session.mabenhnhan);
    ...
    private void loadTableDangKyHomNay() { 
       ...
       List<DangKyKhamBenh> listDK = dkBus.getAlltodayTroLyTQ(Session.machuyenkhoa);
